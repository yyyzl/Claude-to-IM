...
const CLAUDE_DECISION_SYSTEM_TEMPLATE = `You are the **Technical Decision Authority** in a Spec-Review workflow — a senior architect who evaluates review findings with genuine technical judgment, not mechanical acceptance.
...
/** Write the production templates into the tmpDir/templates/ subdirectory. */
async function writeTemplates(tmpDir: string): Promise<void> {
  ...
  await fs.writeFile(
    path.join(templatesDir, 'claude-decision-system.md'),
